...
survival_analysis_by_group <- function(data, time_var, status_var, group_var,
  ...
  if (!requireNamespace("survival", quietly = TRUE)) {
    stop("Please install the 'survival' package: install.packages('survival')")
  }
  data <- as.data.frame(data)

  data <- na.omit(data[c(time_var, status_var, group_var)])
  ...
  cox_formula <- as.formula(paste("surv_obj ~", group_var))
  cox_model <- survival::coxph(cox_formula, data = data)
  cox_summary <- summary(cox_model)

  HR <- sprintf(paste0('%.', digits, 'f'), cox_summary$coefficients[, 2])
  p_value_cox <- cox_summary$coefficients[, 5]

  CI_lower <- sprintf(paste0('%.', digits, 'f'), cox_summary$conf.int[, 3])
  CI_upper <- sprintf(paste0('%.', digits, 'f'), cox_summary$conf.int[, 4])
  HR_CI <- paste0(HR, " (", CI_lower, "-", CI_upper, ")")

  PH_test <- survival::cox.zph(cox_model)

  km_formula <- as.formula(paste("surv_obj ~", group_var))
  km_fit <- survival::survfit(km_formula, data = data)

  logrank_test <- survival::survdiff(km_formula, data = data)
  p_val_logrank <- 1 - pchisq(logrank_test$chisq, length(logrank_test$n) - 1)

  km_summary <- summary(km_fit)
  median_survival <- vector("list", length(group_levels))
  names(median_survival) <- group_levels

  for (i in seq_along(group_levels)) {
    group_name <- group_levels[i]
    group_data <- data[data[[group_var]] == group_name, ]

    event_count <- sum(group_data[[status_var]], na.rm = TRUE)
    total_count <- nrow(group_data)
    event_percent <- sprintf(paste0('%.', digits, 'f'), event_count / total_count * 100)

    if (i <= nrow(km_summary$table)) {
      median_time <- km_summary$table[i, "median"]
      ci_lower <- km_summary$table[i, "0.95LCL"]
      ci_upper <- km_summary$table[i, "0.95UCL"]

      median_time_fmt <- ifelse(is.na(median_time), "NR",
                                sprintf(paste0('%.', digits, 'f'), median_time))
      ci_lower_fmt <- ifelse(is.na(ci_lower), "NR",
                             sprintf(paste0('%.', digits, 'f'), ci_lower))
      ci_upper_fmt <- ifelse(is.na(ci_upper), "NR",
                             sprintf(paste0('%.', digits, 'f'), ci_upper))

      median_survival[[group_name]] <- list(
        events = paste0(event_count, "/", total_count, " (", event_percent, "%)"),
        median = median_time_fmt,
        CI = paste0(ci_lower_fmt, "-", ci_upper_fmt)
      )
    } else {
      median_survival[[group_name]] <- list(
        events = paste0(event_count, "/", total_count, " (", event_percent, "%)"),
  ...
  surv_at_times <- vector("list", length(group_levels))
  names(surv_at_times) <- group_levels

  format_p <- function(p) {
    if (is.na(p)) return("NA")
    if (p < 0.001) "<0.001" else sprintf(paste0('%.', p_digits, 'f'), p)
  }

  max_times <- tapply(data[[time_var]], data[[group_var]], max)

  for (group in group_levels) {
    group_data <- data[data[[group_var]] == group, ]
    group_surv <- survival::Surv(group_data[[time_var]], group_data[[status_var]])

    km_group <- survival::survfit(group_surv ~ 1)

    time_info <- character(length(time_points))

    group_max_time <- max_times[group]

    for (j in seq_along(time_points)) {
      time_point <- time_points[j]

      if (time_point <= group_max_time) {

        km_summary_point <- summary(km_group, times = time_point, extend = FALSE)

        if (length(km_summary_point$surv) > 0 && !is.na(km_summary_point$surv[1])) {
          surv_rate <- km_summary_point$surv[1]
          lower_bound <- km_summary_point$lower[1]
          upper_bound <- km_summary_point$upper[1]

          surv_fmt <- sprintf(paste0('%.', digits, 'f'), surv_rate * 100)
          lower_fmt <- sprintf(paste0('%.', digits, 'f'), lower_bound * 100)
          upper_fmt <- sprintf(paste0('%.', digits, 'f'), upper_bound * 100)

          time_info[j] <- paste0(
            time_point, "m: ", surv_fmt, "% (",
            lower_fmt, "-", upper_fmt, "%)"
          )
        } else {
          time_info[j] <- paste0(time_point, "m: NA")
        }
      } else {
        time_info[j] <- paste0(time_point, "m: NA (", time_point, "m > ",
                               sprintf('%.1f', group_max_time), "m max observed time)")
      }
    }

    surv_at_times[[group]] <- time_info
  }
  ...
  result <- list(
    ...
    Hazard_ratio = HR_CI,
    Cox_p_value = sapply(p_value_cox, format_p),
    Log_rank_p_value = format_p(p_val_logrank),
    Median_survival = median_survival,
    Survival_at_time_points = surv_at_times
  )
  ...
  print(HR_CI)
  cat("\n")
  cat("Cox model p-value:",sapply(p_value_cox, format_p), "\n\n")
  cat("Log-rank Test p-value:", format_p(p_val_logrank), "\n\n")

  cat("Median Survival by Group:\n")
  for (group in group_levels) {
    cat(" - ", group, ": ",
        median_survival[[group]]$events, ", ",
        "Median = ", median_survival[[group]]$median,
        " (", median_survival[[group]]$CI, ")\n", sep = "")
  }

  cat("\nSurvival Rates at Specified Time Points:\n")
  for (group in group_levels) {
    cat(" - ", group, ":\n")
    for (rate in surv_at_times[[group]]) {
      cat("    ", rate, "\n")
    }
  }
